...
namespace raytracing
{
  ...
  void gui::water_section()
  {
    ...
    if (check(ImGui::Checkbox("Render water", &renderWater)))
    {
      ...
      renderWater ? rt::get()->mRender.mGenerateNoise = true : rt::get()->mRender.mGenerateNoise = false;
    }
  ...
  void gui::materials_section()
  {
    ...
    for (size_t i = 0; i < rt::get()->mScene.mMaterialsCount; ++i)
    {
      ...
      if (ImGui::TreeNode(label.c_str()))
      {
        ...
        label = "Alpha###Alpha" + std::to_string(i);
        check(ImGui::DragFloat(label.c_str(), &rt::get()->mScene.mMaterials[i].alpha, 0.01f, 0.0f, 1.0f, "%.2f"));
